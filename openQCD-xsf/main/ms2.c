...
* File ms2.c
*
* Copyright (C) 2012 Martin Luescher, 2014 Mattia Dalla Brida
*
...
static int first,last,step,pmxa,pmxb;
...
static void read_lat_parms(void)
{
   ...
   if (my_rank==0)
   {
      ...
      read_line("pmxa","%d",&pmxa);
      read_line("pmxb","%d",&pmxb);

      error_root(((pmxa<1)||(pmxb<1)),1,"read_lat_parms [ms2.c]",
                 "pmxa and pmxb must be at least 1");
   }

   MPI_Bcast(&kappa,1,MPI_DOUBLE,0,MPI_COMM_WORLD);
   MPI_Bcast(&csw,1,MPI_DOUBLE,0,MPI_COMM_WORLD);
   MPI_Bcast(&cF,1,MPI_DOUBLE,0,MPI_COMM_WORLD);
   MPI_Bcast(&pmxa,1,MPI_INT,0,MPI_COMM_WORLD);
   MPI_Bcast(&pmxb,1,MPI_INT,0,MPI_COMM_WORLD);
