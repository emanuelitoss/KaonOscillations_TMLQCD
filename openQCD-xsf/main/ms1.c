...
{
   double kappa_u,kappa_s,kappa_c,csw,cF,dF,zF;
   ...
   set_lat_parms(0.0,1.0,kappa_u,kappa_s,kappa_c,csw,1.0,cF,dF,zF);
...
static void set_data(int nc)
{
   ...
   for (irw=0;irw<nrw;irw++)
   {
      ...
      else
      {
         ...
         {
            if (rwp.rwfact==RWTM1)
               lnr[isrc]=rwtm1_xsf(rwp.mu,rwp.isp[0],sqn+isrc,stat);
            else if (rwp.rwfact==RWTM1_EO)
               lnr[isrc]=rwtm1eo_xsf(rwp.mu,rwp.isp[0],sqn+isrc,stat);
            else
