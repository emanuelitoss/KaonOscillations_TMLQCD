...
void mult_phase(int pm)
{
   int ix,k;
   su3_dble *u;
   complex_dble phase[3];
   sf_parms_t sfp;
   ...
   phase[0].re=   cos(sfp.theta[0]/(((double)L1*NPROC1)));
   phase[0].im=pm*sin(sfp.theta[0]/(((double)L1*NPROC1)));
   phase[1].re=   cos(sfp.theta[1]/(((double)L2*NPROC2)));
   phase[1].im=pm*sin(sfp.theta[1]/(((double)L2*NPROC2)));
   phase[2].re=   cos(sfp.theta[2]/(((double)L3*NPROC3)));
   phase[2].im=pm*sin(sfp.theta[2]/(((double)L3*NPROC3)));

   for (ix=(VOLUME/2);ix<VOLUME;ix++)
   {
      for (k=2;k<8;k++)
         cm3x3_mulc(phase+k/2-1,u+k,u+k);
      u+=8;
   }
