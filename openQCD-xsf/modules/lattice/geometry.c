...
#define USE_SFCF_PATCH
#ifdef  USE_SFCF_PATCH
static int index(int *bo,int *bs,int x0,int x1,int x2,int x3)
{
   int y0, y1, y2, y3;
   int xb0, xb1, xb2, xb3;
   int xn0, xn1, xn2, xn3;
   int is, ib, eos, eob, cbv, ioff;

   /* relative coordinates of site within bs[0] x bs[1] x bs[2] x bs[3] lattice block */
   y0 = safe_mod(x0,bs[0]);
   y1 = safe_mod(x1,bs[1]);
   y2 = safe_mod(x2,bs[2]);
   y3 = safe_mod(x3,bs[3]);

   /* relative coordinates of site within cache block */
   xb0 = y0%cbs[0];
   xb1 = y1%cbs[1];
   xb2 = y2%cbs[2];
   xb3 = y3%cbs[3];

   /* coordinates of cache block within cbn[0] x cbn[1] x cbn[2] x cbn[3] cache-block grid */
   xn0 = y0/cbs[0];
   xn1 = y1/cbs[1];
   xn2 = y2/cbs[2];
   xn3 = y3/cbs[3];

   /* lexicographic index of site within cache block */
   is = xb0*cbs[1]*cbs[2]*cbs[3] + xb1*cbs[2]*cbs[3] + xb2*cbs[3] + xb3;

   /* lexicographic index of cache block */
   ib = xn0*cbn[1]*cbn[2]*cbn[3] + xn1*cbn[2]*cbn[3] + xn2*cbn[3] + xn3;

   /* parity of site (in full bs[0] x bs[1] x bs[2] x bs[3] lattice block) */
   eos = (y0+y1+y2+y3)&1;

   /* parity of (first site in) cache block */
   eob = (xn0*cbs[0]+xn1*cbs[1]+xn2*cbs[2]+xn3*cbs[3] + bo[0] + bo[1] + bo[2] + bo[3])&1;

   /* volume of cache block */
   cbv = cbs[0]*cbs[1]*cbs[2]*cbs[3];

   /* offset depends on the number of sites with given parity 
      summed over all previous cache blocks (with index < ib)

      Note: Assuming that cbn[mu]*cbs[mu] = L<mu> is even for mu=1,2,3, then
            ib*cbv is odd only if both, cbv and xn3, are odd. Moreover, the
	    rounding in (ib*cbv)/2 needs to be corrected only if eos!=eob.
   */
   ioff = eos*(bs[0]*bs[1]*bs[2]*bs[3])/2  /* offset for odd sites */
     + (ib*cbv)/2                          /* offset (up to rounding) of cache block */
     + (cbv&1)*(xn3&1)*(eos^eob);          /* correction for odd cache block volume */

   /* within a cache block is/2 enumerates the sites with a definite parity */
   return ioff + is/2;  
}
#else
static int index(int *bo,int *bs,int x0,int x1,int x2,int x3)
