...
int main(int argc,char *argv[])
{
	...
	error_root(NPROC!=1,1,"main [check4_ss.c]",
			"must be run in serial");

	if (my_rank==0)
	{
		flog=freopen("check5_ss.log","w",stdout);
		printf("\n");
	...
	read_gauge("6x7/testconfig_6x7");
	write_gauge("6x7/out.conf");

	mult_phase(1);

	read_spinor("6x7/testvector", psd[0]);
	write_spinor("6x7/testvector_test", psd[0]);

	read_spinor("6x7/Qtestvector", psd[1]);
	...
	write_spinor("6x7/my_solution", psd[3]);
	...
	if (my_rank==0)
	{
		printf("Application of Dw_up: Normalized difference = %.2e\n",sqrt(d));
		printf("(should be around 1*10^(-15) or so)\n\n");
	}


	read_spinor("6x7/Qinvtestvector", psd[1]);
   ...
   mu=-mu;
	...
	if (my_rank==0)
	{
		printf("Solution w/o eo-preconditioning for Dw_up^{-1} :\n");
		printf("status = %d\n",status);
