...
{
   data.corr=malloc(2*file_head.ncorr*file_head.nnoise*file_head.tvals*
                                                          sizeof(complex_dble));
...
static void write_data(void)
{
   ...
   if (my_rank==0)
   {
      ...
      if(endian==BIG_ENDIAN)
      {
         bswap_double(2*file_head.nnoise*file_head.tvals*file_head.ncorr*2,
                      data.corr);
         bswap_int(1,&(data.nc));
      }
      ...
      dataoffset=file_head.nnoise*file_head.tvals*file_head.ncorr;
      for (icorr=0;icorr<file_head.ncorr;icorr++)
      {
         chunk=file_head.nnoise*file_head.tvals*(2-file_head.isreal[icorr]);
         nw+=chunk;
         ...
         {
            for (i=0;i<chunk;i++)
               iw+=fwrite(&(data.corr[dataoffset+icorr*file_head.tvals*file_head.nnoise+i]),
                       sizeof(double),1,fdat);
         }else
         {
            iw+=fwrite(&(data.corr[dataoffset+icorr*file_head.tvals*file_head.nnoise]),
                       sizeof(double),chunk,fdat);
         }
      }
      if(endian==BIG_ENDIAN)
