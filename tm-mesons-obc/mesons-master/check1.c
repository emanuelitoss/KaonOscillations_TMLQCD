...
static void write_data(void)
{
   ...
   if (my_rank==0)
   {
      ...
      data.offset=file_head.nnoise*file_head.nnoise*file_head.tvals*file_head.ncorr;
      for (icorr=0;icorr<file_head.ncorr;icorr++)
      {
         chunk=file_head.nnoise*file_head.nnoise*file_head.tvals*(2-file_head.isreal[icorr]);
         nw+=chunk;
         ...
         {
            for (i=0;i<chunk;i++)
               iw+=fwrite(&(data.corr[data.offset+icorr*file_head.tvals*file_head.nnoise*file_head.nnoise+i]),
                       sizeof(double),1,fdat);
         }else
         {
            iw+=fwrite(&(data.corr[data.offset+icorr*file_head.tvals*file_head.nnoise*file_head.nnoise]),
                       sizeof(double),chunk,fdat);
         }
      }
      data.offset=0;
...
static void correlators_contractions(void)
{
   ...
   for(noise_idx1=0;noise_idx1<nnoise;noise_idx1++)
   {
      ...
      for(idx=0;idx<proplist.len_1A;idx++)
      {
         ...
         make_source(eta1,proplist.matrix_typeA[idx],tmp_spinor);
         solve_dirac(proplist.prop_type1[idx],tmp_spinor,xi1[idx],stat,REVERSE_MU);
      ...
      for(idx=0;idx<proplist.len4;idx++)
      {
         ...
         assign_sd2sd(VOLUME,eta1,tmp_spinor);
         solve_dirac(proplist.prop_type4[idx],tmp_spinor,zeta1[idx],stat,STANDARD_MU);
      }
      ...
      for(noise_idx2=0;noise_idx2<nnoise;noise_idx2++)
      {
         ...
         for(idx=0;idx<proplist.len_3C;idx++)
         {
            ...
            make_source(eta2,proplist.matrix_typeC[idx],tmp_spinor);
            solve_dirac(proplist.prop_type3[idx],tmp_spinor,xi2[idx],stat,REVERSE_MU);
         ...
         for(idx=0;idx<proplist.len2;idx++)
         {
            ...
            assign_sd2sd(VOLUME,eta2,tmp_spinor);
            solve_dirac(proplist.prop_type2[idx],tmp_spinor,zeta2[idx],stat,STANDARD_MU);
         }
