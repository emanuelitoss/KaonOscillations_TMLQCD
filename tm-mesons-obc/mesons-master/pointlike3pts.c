...
extern int colour_index(int idx)
{
   ...
   if(idx<0||idx>=N_COLOURS*N_DIRAC)
      error(1,1,"colour_index [pointlike3pts.c]","Invalid index number.");

   colour=idx%N_COLOURS;

   if(colour<0||colour>2)
      error(1,1,"colour_index [pointlike3pts.c]","We are working in SU(3).");
...
extern int dirac_index(int idx)
{
   ...
   if(idx<0||idx>=N_COLOURS*N_DIRAC)
      error(1,1,"[pointlike3pts.c]","Invalid index number.");

   remainder=idx%N_COLOURS;
   result=(int)(idx-remainder)/N_COLOURS;

   if(result<0 || result>3)
      error(1,1,"dirac_index [pointlike3pts.c]","Dirac spinor has 4 components.");
...
static void alloc_data(void)
{
   ...
   error((data.corr==NULL)||(data.corr_tmp==NULL),1,"alloc_data [pointlike3pts.c]", "Unable to allocate data arrays");
}
...
static void write_file_head(void)
{
   ...
   error_root(iw!=6,1,"write_file_head [pointlike3pts.c]", "Incorrect write count");
   ...
   for (i=0;i<file_head.ncorr;i++)
   {
      ...
      error_root(iw!=12,1,"write_file_head [pointlike3pts.c]","Incorrect write count");
   }
}
...
static void check_file_head(void)   
{
   ...
   error_root(ir!=6,1,"check_file_head [pointlike3pts.c]","Incorrect read count");
   error_root(ie!=0,1,"check_file_head [pointlike3pts.c]", "Unexpected value of ncorr, nnoise, tvals or noisetype");
   ...
   for (i=0;i<file_head.ncorr;i++)
   {
      ...
      error_root(ir!=12,1,"check_file_head [pointlike3pts.c]","Incorrect read count");
      error_root(ie!=0,1,"check_file_head [pointlike3pts.c]","Unexpected value of kappa, mu, typeA, typeC, OperatorType");
   }
}
...
static void write_data(void)
{
   ...
   if (my_rank==0)
   {
      ...
      error_root(fdat==NULL,1,"write_data [pointlike3pts.c]","Unable to open dat file");
      ...
      error_root(iw!=nw,1,"write_data [pointlike3pts.c]","Incorrect write count");
      fclose(fdat);
   }
}
...
static int read_data(void)
{
   ...
   error_root(ir!=nr,1,"read_data [pointlike3pts.c]","Read error or incomplete data record");
   if(endian==BIG_ENDIAN)
   {
...
static void read_dirs(void)
{
   if (my_rank==0)
   {
      ...
      error_root((last<first)||(step<1)||(((last-first)%step)!=0),1,
                 "read_dirs [pointlike3pts.c]","Improper configuration range");
...
static void setup_files(void)
{
   if (noexp)
      error_root(name_size("%s/%sn%d_%d",loc_dir,run_name,last,NPROC-1)>=NAME_SIZE,
                 1,"setup_files [pointlike3pts.c]","loc_dir name is too long");
   else if ((!nogauge)&&(!rndmgauge))
      error_root(name_size("%s/%sn%d",cnfg_dir,run_name,last)>=NAME_SIZE,
                 1,"setup_files [pointlike3pts.c]","cnfg_dir name is too long");

   check_dir_root(dat_dir);
   error_root(name_size("%s/%s.mesons.dat~",dat_dir,output_name)>=NAME_SIZE,
              1,"setup_files [pointlike3pts.c]","dat_dir name is too long");

   check_dir_root(log_dir);
   error_root(name_size("%s/%s.mesons.log~",log_dir,output_name)>=NAME_SIZE,
              1,"setup_files [pointlike3pts.c]","log_dir name is too long");
...
static void read_lat_parms(void)
{
   ...
   if (my_rank==0)
   {
      ...
      error_root(((nprop<1)||(ncorr<1)||(nnoise<1)),1,"read_lat_parms [pointlike3pts.c]",
      ...
      error_root(noisetype==-1,1,"read_lat_parms [pointlike3pts.c]",
                 "Unknown noise type");

      read_line("csw","%lf",&csw);
      read_line("cF","%lf",&cF);
      read_line("eoflg","%d",&eoflg);
      error_root(((eoflg!=0)&&(eoflg!=1)),1,"read_lat_parms [pointlike3pts.c]",
		 "Specified eoflg must be 0,1");
      
      read_line("x0","%d",&fixed_x0);
      read_line("z0","%d",&fixed_z0);
      error_root((fixed_x0>N0)||(fixed_x0<0)||(fixed_z0>N0)||(fixed_z0<0)||(fixed_z0>fixed_x0),1,"read_lat_parms [pointlike3pts.c]",
   ...
   error((kappas==NULL)||(mus==NULL)||(isps==NULL)||
         ...
         1,"read_lat_parms [pointlike3pts.c]","Out of memory");
   ...
   if (my_rank==0)
   {
      ...
      for(icorr=0; icorr<ncorr; icorr++)
      {
         ...
         error_root((props1[icorr]<0)||(props1[icorr]>=nprop),1,"read_lat_parms [pointlike3pts.c]",
                 "Propagator index out of range");
         error_root((props2[icorr]<0)||(props2[icorr]>=nprop),1,"read_lat_parms [pointlike3pts.c]",
                 "Propagator index out of range");
         error_root((props3[icorr]<0)||(props3[icorr]>=nprop),1,"read_lat_parms [pointlike3pts.c]",
                 "Propagator index out of range");
         error_root((props4[icorr]<0)||(props4[icorr]>=nprop),1,"read_lat_parms [pointlike3pts.c]",
                 "Propagator index out of range");
         ...
         error_root((typeA[icorr]==-1)||(typeC[icorr]==-1)||(operator_type[icorr]==-1),1,"read_lat_parms [pointlike3pts.c]",
...
static void read_infile(int argc,char *argv[])
{
   ...
   if (my_rank==0)
   {
      ...
      error_root((ifile==0)||(ifile==(argc-1)),1,"read_infile [pointlike3pts.c]",
                 "Syntax: mesons -i <input file> [-noexp] [-nogauge] [-rndmgauge] [-a [-norng]]");

      error_root(endian==UNKNOWN_ENDIAN,1,"read_infile [pointlike3pts.c]",
      ...
      error_root(error_exclusive_options>1,1,"read_infile [pointlike3pts.c]",
            "Invalid flags. Remember that you can use only one between -noexp,-nogauge,-rndmgauge ");

      fin=freopen(argv[ifile+1],"r",stdin);
      error_root(fin==NULL,1,"read_infile [pointlike3pts.c]","Unable to open input file");
   }
   ...
   if (my_rank==0)
   {
      ...
      error_root(fdat==NULL,1,"read_infile [pointlike3pts.c]",
...
static void check_old_log(int *fst,int *lst,int *stp)
{
   ...
   error_root(fend==NULL,1,"check_old_log [pointlike3pts.c]","Unable to open log file");
   ...
   error_root((ie&0x1)!=0x0,1,"check_old_log [pointlike3pts.c]",
              "Incorrect read count");
   error_root((ie&0x2)!=0x0,1,"check_old_log [pointlike3pts.c]",
              "Configuration numbers are not equally spaced");
   error_root(isv==0,1,"check_old_log [pointlike3pts.c]",
...
static void check_old_dat(int fst,int lst,int stp)
{
   ...
   error_root(fdat==NULL,1,"check_old_dat [pointlike3pts.c]",
   ...
   error_root(ic==0,1,"check_old_dat [pointlike3pts.c]","No data records found");
   error_root((ie&0x1)!=0x0,1,"check_old_dat [pointlike3pts.c]","Configuration numbers are not equally spaced");
   error_root((fst!=fc)||(lst!=lc)||(stp!=dc),1,"check_old_dat [pointlike3pts.c]","Configuration range is not as reported in the log file");
}
...
static void check_files(void)
{
   ...
   if (my_rank==0)
   {
      if (append)
      {
         ...
         error_root((fst!=lst)&&(stp!=step),1,"check_files [pointlike3pts.c]",
                    "Continuation run:\n"
                    "Previous run had a different configuration separation");
         error_root(first!=lst+step,1,"check_files [pointlike3pts.c]",
                    "Continuation run:\n"
      ...
      else
      {
         ...
         error_root(fin!=NULL,1,"check_files [pointlike3pts.c]",
                    "Attempt to overwrite old *.log file");
         fdat=fopen(dat_file,"r");
         error_root(fdat!=NULL,1,"check_files [pointlike3pts.c]",
...
static void print_info(void)  
{
   ...
   if (my_rank==0)
   {
      ...
      error_root(flog==NULL,1,"print_info [pointlike3pts.c]",
...
static void make_proplist(void)
{
   ...
   error((proplist.idx_1A==NULL)||(proplist.idx_3C==NULL)||(proplist.idx_2==NULL)||(proplist.idx_4==NULL)
      ...
      ||(proplist.matrix_typeA==NULL)||(proplist.matrix_typeC==NULL),1,"make_proplist [pointlike3pts.c]","Out of memory");
...
static void wsize(int *nws,int *nwsd,int *nwv,int *nwvd) 
{
   ...
   else if (sp.solver==DFL_SAP_GCR)
   {
      ...
      error_root(1,1,"wsize [pointlike3pts.c]","Unknown or unsupported solver");
}
...
static void set_sdble_dirac_color_idx(int vol,spinor_dble *sd,int dc_index)
{
   ...
   for (;sd<sm;sd++)
   {
      ...
      case 3:
         ...
         break;
      default:
         error_root(colour_ptr==NULL,1,"set_sdble_dirac_color_idx [pointlike3pts.c]","Unable to allocate an su3_vector_dble pointer");
         error_root(1,1,"set_sdble_dirac_color_idx [pointlike3pts.c]","Invalid Dirac index");
         break;
      }
      ...
      case 2:
         ...
         break;
      default:
         error_root(1,1,"set_sdble_dirac_color_idx [pointlike3pts.c]","Invalid colour index");
         break;
      }
   }
}
...
static void solve_dirac(int prop, spinor_dble *eta, spinor_dble *psi, int *status, int reverse)  /* modified */
{
   ...
   if (sp.solver==CGNE)
   {
      ...
      tmcg(sp.nmx,sp.res,mu,eta,eta,status);
   ...
   else if (sp.solver==DFL_SAP_GCR)
   {
      ...
      dfl_sap_gcr2(sp.nkv,sp.nmx,sp.res,mu,eta,psi,status);
      if (my_rank==0)
         printf("%i %i\n",status[0],status[1]);
...
void make_source(spinor_dble *eta, int type, spinor_dble *xi)
{
   switch (type) 
   {
      ...
      case ONE_TYPE:
         ...
         break;
      default:
         error_root(1,1,"make_source [pointlike3pts.c]",
...
static void contraction_single_trace(spinor_dble *xi1,spinor_dble *xi2,spinor_dble *zeta1,spinor_dble *zeta2,spinor_dble *psi1,spinor_dble *psi2,int idx_noise1,int idx_noise2,int idx_corr)   /* new function */
{
         ...
         for(mu=0;mu<nu;mu++)
         {
            ...
            switch (type)
            {
               ...
               case TTt:
                  ...
                  break;
               default:
                  error(1,1,"contraction_single_trace [pointlike3pts.c]","Invalid operator type");
                  break;
            }
   ...
   else error(1,1,"contraction_single_trace [pointlike3pts.c]","Invalid Dirac-gamma types");
}
...
static void init_rng(void)
{
   ...
   if (append)
   {
      ...
      else
      {
         ...
         error_root(ic!=(first-step),1,"init_rng [pointlike3pts.c]",
...
static void save_ranlux(void)
{
   ...
   if (rlxs_state==NULL)
   {
      ...
      error(rlxs_state==NULL,1,"save_ranlux [pointlike3pts.c]",
...
int main(int argc,char *argv[])
{
   ...
   for (nc=first;(iend==0)&&(nc<=last);nc+=step)
   {
      ...
      if (dfl.Ns)
      {
         ...
         error_root(status[0]<0,1,"main [pointlike3pts.c]",
