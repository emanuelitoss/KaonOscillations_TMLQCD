...
*
* Syntax: mesons -i <input file> [-noexp] [-a] -nogauge
*
...
static struct
{
   ...
   double *kappa1;   /* hopping parameters for the choosen quark lines */
   double *kappa2;
/* DP */ 
   double *mus1;  /* twisted mass for the choosen quark lines */
   double *mus2;
   double *mus3;
   double *mus4;
/* DP */
   int *typeA; /* dirac matrices */
   int *typeB;
   int *typeC;
   int *typeD;
   int *x0; /* x0 values */
...
static struct
{
   ...
   int **prop_z0;   /* propagator index of each x0 and propagator */
   int **type_z0;   /* type index of each x0 and propagator  */
} proplist;
...
static int *isps,*props1,*props2,*typeA,*typeB,*x0s;
...
/* DP */
      error_root(iw!=8,1,"write_file_head [mesons.c]",
              "Incorrect write count");
/* DP */
   }
}
...
/* DP */
      error_root(ir!=8,1,"check_file_head [mesons.c]",
              "Incorrect read count");
/* DP */
      error_root(ie!=0,1,"check_file_head [mesons.c]",
...
/* DP */
      ...
      if ((typeA[icorr]==GAMMA5_TYPE)&&(typeB[icorr]==GAMMA5_TYPE)&&
...
static void make_proplist(void)
{
   ...
   for (i=0;i<proplist.nux0;i++)
   {
      ...
      for (icorr=0;icorr<ncorr;icorr++)
      {
         ...
         {
            if (!kappatype[typeA[icorr]+MAX_TYPE*props2[icorr]])
            {
               kappatype[typeA[icorr]+MAX_TYPE*props2[icorr]]=1;
               proplist.nprop[i]++;
            }
      ...
      for (inoise=0;inoise<nnoise;inoise++)
      {
         ...
         for (icorr=0;icorr<ncorr;icorr++)
         {
            if (x0s[icorr]==proplist.ux0[ix0])
            {
               ...
               for (iprop=0;iprop<proplist.nprop[ix0];iprop++)
               {
                  if ((typeA[icorr]==proplist.type[ix0][iprop])&&
               ...
               make_xi(zeta[ip1],typeB[icorr],xi);
